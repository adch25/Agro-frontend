...
const HydroMeteorologicalInstruments = () => {
              ...
              <td>${
                hmi.hydroMeteorological_videos?.length > 0
                  ? hmi.hydroMeteorological_videos
                      .map(
                        (video, index) =>
                          `<a href="${process.env.EXPO_PUBLIC_API_BACKEND_API_URL}/${video}">Video ${
                            index + 1
                          }</a>`
                      )
                      .join("<br/>")
  ...
  const getUniqueFilePath = async (basePath, filename, extension) => {
    let finalPath = `${basePath}${filename}.${extension}`;
    let counter = 1;

    while (await FileSystem.getInfoAsync(finalPath).exists) {
      finalPath = `${basePath}${filename}_${counter}.${extension}`;
      counter++;
    }
  ...
  const downloadSingleEntryAsPDF = async (item, filename) => {
    try {
      ...
      const baseDir = `${FileSystem.documentDirectory}HydroMeteoReports/`;
      ...
      setAlertMessage(
        `PDF saved to ${destinationPath}. On Android, use a file manager to access it. On iOS, check the Files app under "On My iPhone".`
      );
      setShowAlert(true);
    } catch (error) {
      console.error("Error downloading PDF:", error);
      setAlertMessage(`Failed to download PDF: ${error.message}`);
      setShowAlert(true);
    }
  };
  ...
  const downloadAllEntriesAsPDF = async (entries) => {
    ...
    try {
      ...
      // Generate and save individual PDFs
      const pdfPaths = [];
      for (let index = 0; index < entries.length; index++) {
        const hmi = entries[index];
        const filename = `hydro_meteorological_instrument_${index + 1}`;
        const pdfUri = await generatePDF(hmi, filename);
        if (pdfUri) {
          const destinationPath = await getUniqueFilePath(
            baseDir,
            filename,
          ...
          pdfPaths.push(destinationPath);
        }
      }

      if (pdfPaths.length === 0) {
        setAlertMessage("Failed to generate any PDFs.");
        setShowAlert(true);
        return;
      }

      setAlertMessage(
        `Saved ${pdfPaths.length} PDFs to ${baseDir}. ZIP creation is not supported natively. On Android, use a file manager to access them. On iOS, check the Files app under "On My iPhone". To create a ZIP, please use a backend service or manually zip the files.`
      );
      setShowAlert(true);
    } catch (error) {
      console.error("Error downloading all entries:", error);
      setAlertMessage(`Failed to download entries: ${error.message}`);
      setShowAlert(true);
    } finally {
      setIsLoading(false);
    }
